...
服务端仍保留六个相互隔离的动态入口和 latest report 槽位；条件变化后，旧
报告必须与新的 protocol、selection 和 parameters 完全匹配才可继续投影。
老版本切换只改变 Flow 结构并标记为概念对照，运行按钮禁用且不会自动切回
v2；真实 Legacy 路线通过协议入口选择。服务端的 25 个自动化用例继续作为
独立门禁执行。
